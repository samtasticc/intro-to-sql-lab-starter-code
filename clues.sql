...
-- Write SQL query here
 world=# SELECT name FROM countries WHERE region = 'Southern Europe' AND population <3000;
 Holy See (Vatican City State)

-- Clue #2: Now that we're here, we have insight that Carmen was seen attending language classes in this country's officially recognized language. Check our databases and find out what language is spoken in this country, so we can call in a translator to work with you.

-- Write SQL query here
world=# SELECT code FROM countries WHERE region = 'Southern Europe' AND population <3000;
 VAT
...
-- Write SQL query here
world=# SELECT countries.code, countries.name, countries.region, cities.name
world-# FROM countries
world-# JOIN cities
world-# ON countries.code = cities.countrycode
world-# WHERE region = 'South America' AND cities.name LIKE '%San Mar%';
 code |   name    |    region     |        name
------+-----------+---------------+---------------------
 ARG  | Argentina | South America | General San Mart├¡n
(1 row)
